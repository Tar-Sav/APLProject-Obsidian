...
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), "APLProject-Obsidian"))
...
def _load_cfg():
    ...
    c.read(CONFIG_PATH)
    api_key = c.get("openai", "api_key", fallback="")
...
def _call_openai(code):
    try:
        from openai import OpenAI, RateLimitError
    except ModuleNotFoundError:
        return "ERROR: openai package not found.\nRun: pip install openai"
    ...
    for model in OPENAI_MODELS:
        try:
            ...
            return f"ERROR contacting OpenAI ({model}): {e}"
    return "ERROR: All OpenAI models hit rate limits. Please wait and try again."
...
def open_ai_window():
    ...
    w, h = 500, min(root.winfo_height(), sh - 80)
    x = min(root.winfo_x() + root.winfo_width() + 6, sw - w - 10)
    y = max(root.winfo_y(), 40)
    ai_win.geometry(f"{w}x{h}+{x}+{y}")
    ...
    def _set_key():
        global api_key
        k = simpledialog.askstring("OpenAI API Key",
